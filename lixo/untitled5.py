...
while 1>0:
    ...
    if combustivel=="gasolina":
        ...
        a4=-42.2764373650608
        a5=1.35175803975684
        mol=((12.0107*c+hid*1.008+16*ox)+AC*(32+3.76*28.013))/(1+AC+AC*3.76)  #massa molecular da mistura
        break
...
def motorComb(t,x,Pm):
    ...
    k=wiebe(t)*KProd(T)+(1-wiebe(t))*KReag(T)
    dkdT=wiebe(t)*dkprod(T)+(1-wiebe(t))*dkreag(T)
...
for j in range(len(tk))[len(ti)-1:len(ti)+len(tj)-1]: #199 ao 198
    ...
    x=solve_ivp(motorComb,ts,x0,method='DOP853',args=(P1[j],)).y
    ...
    wiebeEv.append(wiebe(tk[j+1]))
 

for f in range(len(tk))[len(tj)+len(ti)-1:len(tk)-1]: #200 ao 298
    ts=[tk[f],tk[f+1]]
    
    x=solve_ivp(motorComb,ts,x0,method='DOP853',args=(P1[f],)).y
