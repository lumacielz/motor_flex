...
def twoStepKinects (Concentrations0,ts,Temperature,Pressure):
    ...
    print(odeint(dc,Concentrations0,ts,args=(Temperature,)))
    print(concentration)
...
tk=np.linspace(teta0,tetaf,100)
#tk[:len(ti)]=ti
...
for j in range (len(ti)-1,len(ti)+len(tj)-1):
    ...
    x=solve_ivp(motorComb,ts,x0,args=(P1[j],xt,dxdt),method='DOP853',atol=1e-10,rtol=1e-10).y
