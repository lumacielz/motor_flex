...
# -*- coding: utf-8 -*-
"""
Created on Wed Mar 30 12:23:09 2022
...
def temperaturaAdiabaticaV2(Tx,composicao):
    Ncomb=composicao[0]
    NO2=composicao[1]
    NH2O=composicao[2]
    NCO2=composicao[3]
    ...
    h_comb,h_o2,h_h2o,h_co2,h_co,h_o,h_n,h_no2,h_no,h_oh,h_h2,h_h,h_n2=interpolacaoH(Tx)
    entalpy_products0=Ncomb*h_comb+NO2*h_o2+NH2O*h_h2o+NCO2*h_co2+NCO*h_co+NO*h_o+NN*h_n+NNO2*h_no2+NNO*h_no+NOH*h_oh+NH2*h_h2+NH*h_h+NN2*h_n2
    zero=entalpy_products0-entalpy_reactants
    
    return zero

def entalpyConstraint(composicoes, T):
    entalpy_products = 0
    for c,h in zip(composicoes,interpolacaoH(T)):
        entalpy_products +=c*h
    return entalpy_reactants - entalpy_products

def composicaoAdiabatica(estimative,T,P,entalpy_reactants):
    start = time.time()
    while True:
        sol = minimize(gibbs, estimative,args=(T,P), method='SLSQP', bounds=bnds, constraints=cons)
        ...
        nextTemp=float(temperaturaAdiabatica(sol.x,T,T+2,entalpy_reactants))
    ...
    print(end-start)     
    return nextTemp,P,sol.x,total_mols

def constraintV2(T):
    return [{'type': 'eq', 'fun': restricaoC},{'type': 'eq', 'fun': restricaoO},{'type': 'eq', 'fun':restricaoH},{'type': 'eq', 'fun': restricaoN},{'type': 'eq', 'fun': entalpyConstraint, 'args': (T,)}]

def composicaoAdiabaticaV2(estimative,T,P,entalpy_reactants):
    start = time.time()
    while True:
        consA = constraintV2(T)
        
        sol = minimize(gibbs, estimative,args=(T,P), method='SLSQP', bounds=bnds, constraints=consA)
        
        total_mols = sum(i for i in sol.x)
        
        nextTemp = fsolve(temperaturaAdiabaticaV2,T,args=(sol.x,))
        #nextTemp=float(temperaturaAdiabatica(sol.x,T,T+2,entalpy_reactants))
...
#
    adiab=composicaoAdiabatica(estimative,T,P,entalpy_reactants)
