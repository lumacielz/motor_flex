...
#----------------------------------EQUILIBRIO 2D------------------------------------#
    ...
    for j in range (len(ti)-1,len(ti)+len(tj)-1):
        ...
        burned = equilibrioAdiabatico(estimative, T[j],P[j]*9.869*10**-6,composition0,T[j])
        burnedComposition = burned[1]
        next_composition = wiebe(tk[j])*burnedComposition + (1-wiebe(tk[j]))*reactants_composition
    ...
    for f in range(len(tj)+len(ti)-1,len(tk)-1): #200 ao 298
        ...
        burnedComposition = equilibrioAdiabatico(estimative, T[f],P[f]*9.869*10**-6,composition0, T[f])[1]
        next_composition = wiebe(tk[f])*burnedComposition + (1-wiebe(tk[f]))*reactants_composition
...
###plots de graficos
    ...
    ax1[1].plot(tkd,T)
    ax1[2].plot(tkd[len(ti)-2:-1],massFraction)
    ax2[0].plot(tkd,Qa)
    ax2[1].plot(tkd,Qp)
    ax2[2].plot(tkd,W)
